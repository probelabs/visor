...
async function main() {
    try {
        ...
        if (!isUsingConfigChecks) {
            const { invalid: invalidChecks } = check_execution_engine_1.CheckExecutionEngine.validateCheckTypes(checksToRun);
        ...
        try {
            ...
            for (const checkName of checksToRun) {
                try {
                    const failureResults = await executionEngine.evaluateFailureConditions(checkName, analysisResult.reviewSummary, config);
                    ...
                    const logFn = mergedConfig.cliOutput === 'json' || mergedConfig.cliOutput === 'sarif'
                        ? console.error
                        : console.log;
                    logFn(`⚠️  Warning: Failed to evaluate failure conditions for check '${checkName}': ${error instanceof Error ? error.message : 'Unknown error'}`);
                }
            }
