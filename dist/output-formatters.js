...
class OutputFormatters {
    ...
    static formatAsTable(result, options = {}) {
        ...
        const overallScore = (0, reviewer_1.calculateOverallScore)(issues);
        // Summary table
        ...
        summaryTable.push(['Overall Score', `${overallScore}/100`], ['Total Issues', totalIssues.toString()], ['Critical Issues', criticalIssues.toString()], ['Files Analyzed', result.repositoryInfo.files.length.toString()], ['Total Additions', result.repositoryInfo.totalAdditions.toString()], ['Total Deletions', result.repositoryInfo.totalDeletions.toString()], ['Execution Time', `${result.executionTime}ms`], ['Checks Executed', result.checksExecuted.join(', ')]);
        ...
        if (issues.length > 0) {
            if (groupByCategory) {
                const groupedComments = this.groupCommentsByCategory((0, reviewer_1.convertIssuesToComments)(issues));
    ...
    static formatAsSarif(result, _options = {}) {
        ...
        const sarifResults = issues.map((issue, _index) => {
    ...
    static groupCommentsByCategory(comments) {
        const grouped = {
            security: [],
            performance: [],
            style: [],
    ...
    static calculateCategoryScore(comments) {
        if (comments.length === 0)
            return 100;
     ...
     */
    static calculateOverallScore(issues) {
        if (issues.length === 0)
            return 100;
    ...
    static getFileStatusEmoji(status) {
        const emojiMap = {
            added: '✅',
            removed: '❌',
            modified: '📝',
