...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-TXP7J4IC.mjs");
...
var GitHubOpsProvider = class extends CheckProvider {
  ...
  async execute(prInfo, config, dependencyResults) {
    ...
    const renderValues = async (arr) => {
      ...
      const liq = createExtendedLiquid({ cache: false, strictFilters: false, strictVariables: false });
      const outputs = {};
      if (dependencyResults) {
...
var CheckExecutionEngine = class _CheckExecutionEngine {
  ...
  getRoutingSandbox() {
    ...
    this.routingSandbox = new Sandbox4({ globals, prototypeWhitelist });
...
${expr}
  ...
  async evaluateCheckCondition(checkName, condition, prInfo, results, debug) {
    ...
    const commenterAssoc = prInfo?.eventContext?.comment?.author_association || prInfo?.eventContext?.comment?.authorAssociation || prInfo?.eventContext?.issue?.author_association || prInfo.authorAssociation;
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    ...
    const { createExtendedLiquid: createExtendedLiquid2 } = await import("./liquid-extensions-VX3LH7ET.mjs");
    ...
    if (enrichAssistantContext) {
      ...
      try {
        ...
        authorAssociation = anyInfo?.eventContext?.comment?.author_association || anyInfo?.authorAssociation;
    ...
    const { withPermissionsContext } = await import("./liquid-extensions-VX3LH7ET.mjs");
    let authorAssociationForFilters;
    try {
      const anyInfo = _prInfo;
      authorAssociationForFilters = anyInfo?.eventContext?.comment?.author_association || anyInfo?.authorAssociation;
    } catch {
    }
    let rendered;
...
//# sourceMappingURL=chunk-26KPYO5L.mjs.map