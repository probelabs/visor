      ...
      async cloneSession(sourceSessionId, newSessionId, checkName) {
        ...
        try {
          ...
          if (sourceAgent.debug && checkName) {
            try {
              const { initializeTracer: initializeTracer2 } = await import("./tracer-init-GTYIXLVL.mjs");
...
`;
  ...
  parseAIResponse(response, debugInfo, _schema) {
    ...
    try {
      ...
      log(`\u{1F50D} Schema detection: _schema="${_schema}", isCustomSchema=${isCustomSchema}`);
      if (_schema) {
...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-V3KCJP2A.mjs");
...
function compileAndRun(sandbox, userCode, scope, opts = { injectLog: true, wrapFunction: true, logPrefix: "[sandbox]" }) {
  const header = opts.injectLog ? `const log = (...a) => { try { console.log('${opts.logPrefix ?? "[sandbox]"}', ...a); } catch {} };
` : "";
...
//# sourceMappingURL=chunk-ECUARACH.mjs.map