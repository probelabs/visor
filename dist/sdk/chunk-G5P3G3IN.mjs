...
async function acquirePromptLock() {
  ...
  await new Promise((resolve4) => waiters.push(resolve4));
  activePrompt = true;
}
  ...
  return new Promise((resolve4, reject) => {
    ...
    if (options.timeout && options.timeout > 0) {
      timeoutId = setTimeout(() => {
        cleanup();
        if (defaultValue !== void 0) return resolve4(defaultValue);
...
async function handlePlanReady(context2, state, transition) {
  ...
  const requestedChecksSet = context2.requestedChecks ? new Set(context2.requestedChecks) : void 0;
  ...
  for (const [checkId, checkConfig] of Object.entries(context2.config.checks || {})) {
    if (requestedChecksSet && !requestedChecksSet.has(checkId)) {
      if (context2.debug) {
        logger.info(`[PlanReady] Skipping check '${checkId}': not in requested checks list`);
      }
      continue;
    }
...
async function executeSingleCheck(checkId, context2, state, emitEvent, transition) {
  ...
  if (forEachParent && forEachItems && forEachItems.length > 0) {
  ...
  try {
    ...
    if (checkConfig2.forEach === true) {
      ...
      if (Array.isArray(output)) {
        ...
        if (context2.debug) {
          logger.warn(`[LevelDispatch] Check ${checkId} has forEach:true but output is not an array: ${typeof output}`);
        }
      }
    }
    if (result.isForEach) {
...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-U7NNK7UW.mjs");
                ...
                try {
                  ...
                  if (typeof sc === "string") {
                    ...
                    try {
                      ...
                      const path9 = __require("path");
                      const candidates = [
                        path9.join(__dirname, "output", name, "schema.json"),
                        path9.join(process.cwd(), "output", name, "schema.json")
                      ];
                      for (const p of candidates) {
                        try {
                          const txt = fs7.readFileSync(p, "utf8");
...
${expr}`;
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    ...
    const { createExtendedLiquid: createExtendedLiquid2 } = await import("./liquid-extensions-MSVAERPV.mjs");
    ...
    const { withPermissionsContext } = await import("./liquid-extensions-MSVAERPV.mjs");
...
//# sourceMappingURL=chunk-G5P3G3IN.mjs.map