    ...
    AICheckProvider = class extends CheckProvider {
      ...
      async executeWithConfig(prInfo, config, _dependencyResults, sessionInfo) {
        ...
        try {
          ...
          logger.debug(`[AI Provider] Workspace detection for check '${config.checkName || "unknown"}':`);
          ...
          if (workspace && typeof workspace.isEnabled === "function" && workspace.isEnabled()) {
            ...
            const originalDir = parentCtx?.originalWorkingDirectory;
            const filtered = folders.filter((p) => {
              if (typeof p !== "string" || !p) return false;
              if (originalDir && p === originalDir && workspaceRoot && !p.startsWith(workspaceRoot)) {
                logger.debug(`[AI Provider] Filtering out original directory from allowedFolders: ${p}`);
                return false;
              }
              return true;
            });
            ...
            if (unique.length > 0 && workspaceRoot) {
              ...
              logger.debug(`[AI Provider]   originalWorkingDirectory (excluded): ${originalDir || "N/A"}`);
...
async function renderTemplateContent(checkId, checkConfig, reviewSummary) {
  try {
    const { createExtendedLiquid: createExtendedLiquid2 } = await import("./liquid-extensions-DL6Z5IUT.mjs");
  ...
  if (parentCheckConfig?.on_finish && parentCheckConfig.forEach) {
    ...
    try {
      ...
      if (parentResult) {
        ...
        try {
          const { evaluateTransitions } = await import("./routing-JOTBYI3M.mjs");
        ...
        const { evaluateGoto: evaluateGoto2 } = await import("./routing-JOTBYI3M.mjs");
        if (context2.debug) {
          logger.info(
            `[LevelDispatch] Evaluating on_finish.goto_js for forEach parent: ${forEachParent}`
          );
  ...
  "src/providers/workflow-check-provider.ts"() {
    ...
    WorkflowCheckProvider = class extends CheckProvider {
      ...
      async prepareInputs(workflow, config, prInfo, dependencyResults) {
        ...
        logger.debug(`[WorkflowProvider] eventContext.conversation: ${eventContext.conversation ? "present" : "absent"}`);
        const slack = (() => {
          try {
            const anyCtx = eventContext;
        ...
        if (conversation) {
          logger.debug(`[WorkflowProvider] conversation.messages count: ${Array.isArray(conversation.messages) ? conversation.messages.length : 0}`);
        }
...
//# sourceMappingURL=chunk-5DA3TR4V.mjs.map