...
async function executeWorkflowAsTool(workflowId, args, context2, argsOverrides) {
  ...
  const { WorkflowCheckProvider: WorkflowCheckProvider2 } = await import("./workflow-check-provider-H3CUOLUD.mjs");
    ...
    if (code) {
      try {
        ...
        if (process.env.VISOR_TRACE_REPORT === "true") {
          ...
          try {
            if (!fs7.existsSync(outDir)) fs7.mkdirSync(outDir, { recursive: true });
            const ts = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-");
            const jsonPath = path8.join(outDir, `${ts}.trace.json`);
            const htmlPath = path8.join(outDir, `${ts}.report.html`);
            let data = { spans: [] };
            if (fs7.existsSync(jsonPath)) {
              try {
                data = JSON.parse(fs7.readFileSync(jsonPath, "utf8"));
              } catch {
                data = { spans: [] };
              }
            }
            ...
            fs7.writeFileSync(jsonPath, JSON.stringify(data, null, 2), "utf8");
            if (!fs7.existsSync(htmlPath)) {
              fs7.writeFileSync(
                htmlPath,
  ...
  if (parentCheckConfig?.on_finish && parentCheckConfig.forEach) {
    ...
    try {
      ...
      if (parentResult) {
        ...
        try {
          const { evaluateTransitions } = await import("./routing-6R42GXUO.mjs");
        ...
        const { evaluateGoto: evaluateGoto2 } = await import("./routing-6R42GXUO.mjs");
        if (context2.debug) {
          logger.info(
            `[LevelDispatch] Evaluating on_finish.goto_js for forEach parent: ${forEachParent}`
          );
  ...
  "src/providers/workflow-check-provider.ts"() {
    ...
    WorkflowCheckProvider = class extends CheckProvider {
      ...
      async loadWorkflowFromConfigPath(sourcePath, baseDir) {
        ...
        const rawContent = fs8.readFileSync(resolved, "utf8");
...
//# sourceMappingURL=chunk-PJ7K5UFC.mjs.map