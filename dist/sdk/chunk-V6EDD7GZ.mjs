...
`;
              readableVersion += `${"=".repeat(60)}
`;
              const debugArtifactsDir = process.env.VISOR_DEBUG_ARTIFACTS || path7.join(process.cwd(), "debug-artifacts");
              if (!fs5.existsSync(debugArtifactsDir)) {
                fs5.mkdirSync(debugArtifactsDir, { recursive: true });
              }
              const debugFile = path7.join(
                debugArtifactsDir,
                `prompt-${_checkName || "unknown"}-${timestamp}.json`
              );
              fs5.writeFileSync(debugFile, debugJson, "utf-8");
              const readableFile = path7.join(
                debugArtifactsDir,
...
\u{1F4BE} Full debug info saved to:`);
          ...
          if (process.env.VISOR_DEBUG_AI_SESSIONS === "true") {
            try {
              ...
              fs5.writeFileSync(sessionBase + ".json", JSON.stringify(sessionData, null, 2), "utf-8");
              let readable = `=============================================================
`;
...
`;
              ...
              fs5.writeFileSync(sessionBase + ".summary.txt", readable, "utf-8");
...
`;
              ...
              const promptFile = path7.join(tempDir, `visor-prompt-${timestamp}.txt`);
...
\u{1F4BE} Prompt saved to: ${promptFile}`);
              ...
              try {
                ...
                fs5.writeFileSync(base + ".json", debugJson, "utf-8");
                fs5.writeFileSync(base + ".summary.txt", readableVersion, "utf-8");
                log(`
...
`;
      ...
      async loadSchemaContent(schema) {
        ...
        if ((schema.startsWith("./") || schema.includes(".json")) && !path7.isAbsolute(schema)) {
          if (schema.includes("..") || schema.includes("\0")) {
            throw new Error("Invalid schema path: path traversal not allowed");
          }
          try {
            const schemaPath = path7.resolve(process.cwd(), schema);
        ...
        const candidatePaths = [
          ...
          path7.join(__dirname, "output", sanitizedSchemaName, "schema.json"),
          // Historical fallback when src/output was inadvertently bundled as output1/
          path7.join(__dirname, "output1", sanitizedSchemaName, "schema.json"),
          // Local dev (repo root)
          path7.join(process.cwd(), "output", sanitizedSchemaName, "schema.json")
        ];
        ...
        const distPath = path7.join(__dirname, "output", sanitizedSchemaName, "schema.json");
        const distAltPath = path7.join(__dirname, "output1", sanitizedSchemaName, "schema.json");
        const cwdPath = path7.join(process.cwd(), "output", sanitizedSchemaName, "schema.json");
        throw new Error(
    ...
    HumanInputCheckProvider = class _HumanInputCheckProvider extends CheckProvider {
      ...
      async tryReadFile(filePath) {
        try {
          ...
          try {
            await fs4.promises.access(normalizedPath, fs4.constants.R_OK);
            const stats = await fs4.promises.stat(normalizedPath);
            if (!stats.isFile()) {
              return null;
            }
            const content = await fs4.promises.readFile(normalizedPath, "utf-8");
            return content.trim();
          } catch {
            return null;
          }
  ...
  const expandWithTransitives = (rootChecks) => {
    ...
    const visit = (checkId) => {
      ...
      for (const depId of deps) {
        if (!context2.config.checks?.[depId]) continue;
...
async function executeCheckWithForEachItems(checkId, forEachParent, forEachItems, context2, state, emitEvent, transition) {
  ...
  for (let itemIndex = 0; itemIndex < forEachItems.length; itemIndex++) {
    ...
    try {
      ...
      const output = itemResult.output;
      allOutputs.push(output);
      const content = itemResult.content;
...
async function executeSingleCheck(checkId, context2, state, emitEvent, transition) {
  ...
  try {
    ...
    const outputWithTimestamp = result.output !== void 0 ? { ...result.output, ts: Date.now() } : void 0;
    const enrichedResultWithTimestamp = outputWithTimestamp ? { ...enrichedResult, output: outputWithTimestamp } : enrichedResult;
    try {
      context2.journal.commitEntry({
        sessionId: context2.sessionId,
        checkId,
...
function updateStats(results, state) {
...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-KC4UDU37.mjs");
...
`;
  ...
  saveDebugArtifact(debug) {
    try {
      const fs5 = __require("fs");
      const path7 = __require("path");
      const debugDir = path7.join(process.cwd(), "debug-artifacts");
...
var PRAnalyzer = class {
  constructor(octokit, maxRetries = 3, workingDirectory = path2.resolve(process.cwd())) {
    this.octokit = octokit;
...
var CheckExecutionEngine = class _CheckExecutionEngine {
  ...
  engineMode;
  constructor(workingDirectory, octokit, engineMode) {
                ...
                try {
                  ...
                  if (typeof sc === "string") {
                    ...
                    try {
                      ...
                      const path7 = __require("path");
                      const candidates = [
                        path7.join(__dirname, "output", name, "schema.json"),
                        path7.join(process.cwd(), "output", name, "schema.json")
                      ];
                      for (const p of candidates) {
                        try {
                          const txt = fs5.readFileSync(p, "utf8");
...
${expr}`;
  ...
  async executeGroupedChecks(prInfo, checks, timeout, config, outputFormat, debug, maxParallelism, failFast, tagFilter, _pauseGate) {
    ...
    if (debug) {
      ...
      logger.debug(`\u{1F527} Debug: Engine mode: ${this.engineMode}`);
    }
    try {
  ...
  async validateTemplatePath(templatePath) {
    ...
    const resolvedPath = path7.resolve(projectRoot, templatePath);
    ...
    if (!resolvedPath.startsWith(resolvedProjectRoot + path7.sep) && resolvedPath !== resolvedProjectRoot) {
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    ...
    const { createExtendedLiquid: createExtendedLiquid2 } = await import("./liquid-extensions-LB5KGCBA.mjs");
    ...
    if (checkConfig.template) {
      ...
      for (const p of candidateTemplatePaths) {
        try {
          templateContent = await fs5.readFile(p, "utf-8");
          foundTemplate = p;
          break;
        } catch {
        }
      ...
      if (!foundTemplate) {
        const distPath = path7.join(__dirname, `output/${sanitizedSchema}/template.liquid`);
        const cwdPath = path7.join(process.cwd(), `output/${sanitizedSchema}/template.liquid`);
        throw new Error(
    ...
    const { withPermissionsContext } = await import("./liquid-extensions-LB5KGCBA.mjs");
    ...
    try {
      const { emitMermaidFromMarkdown: emitMermaidFromMarkdown2 } = await import("./mermaid-telemetry-4LSC3TKJ.mjs");
...
//# sourceMappingURL=chunk-V6EDD7GZ.mjs.map