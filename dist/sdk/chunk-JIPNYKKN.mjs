  ...
  "src/state-machine/runner.ts"() {
    ...
    StateMachineRunner = class {
      ...
      buildExecutionResult() {
        ...
        try {
          const snapshotId = this.context.journal.beginSnapshot();
          const visible = this.context.journal.readVisible(
            this.context.sessionId,
            snapshotId,
            this.context.event
          );
          const byCheck = {};
          const childRunsByCheck = {};
          for (const e of visible) {
            try {
              if (Array.isArray(e.scope) && e.scope.length === 0) {
                const r = e.result;
                if (r && r.isForEach === true && Array.isArray(r.forEachItems)) {
                  byCheck[e.checkId] = (byCheck[e.checkId] || 0) + 1;
                }
                if (r && r.isForEach === true && Array.isArray(r.forEachItemResults)) {
                  childRunsByCheck[e.checkId] = r.forEachItemResults.length;
                }
              }
            } catch {
            }
          ...
          for (const s of stats) {
            const cfg = this.context.config.checks?.[s.checkName];
            if (cfg?.forEach === true && byCheck[s.checkName] !== void 0) {
              const runs = byCheck[s.checkName];
              s.totalRuns = runs;
              const totalSF = (s.successfulRuns || 0) + (s.failedRuns || 0);
              if (totalSF < runs) {
                s.successfulRuns = runs;
                s.failedRuns = 0;
              } else if (totalSF > runs) {
                const ratio = runs / (totalSF || 1);
                s.successfulRuns = Math.floor((s.successfulRuns || 0) * ratio);
                s.failedRuns = runs - s.successfulRuns;
              }
            } else if (!cfg?.forEach && childRunsByCheck[s.checkName] !== void 0) {
              const runs = childRunsByCheck[s.checkName];
              s.totalRuns = runs;
              const totalSF = (s.successfulRuns || 0) + (s.failedRuns || 0);
              if (totalSF === 0) {
                s.successfulRuns = runs;
  ...
  async executeGroupedChecks(prInfo, checks, timeout, config, outputFormat, debug, maxParallelism, failFast, tagFilter, _pauseGate) {
    ...
    try {
      if (this.executionContext?.mode?.postGroupedComments && configWithTagFilter?.output?.pr_comment) {
        const { PRReviewer } = await import("./reviewer-TKZLT62U.mjs");
...
//# sourceMappingURL=chunk-JIPNYKKN.mjs.map