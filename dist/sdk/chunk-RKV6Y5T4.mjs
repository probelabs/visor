...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-T2UYXZGU.mjs");
...
var AICheckProvider = class extends CheckProvider {
  ...
  async executeWithConfig(prInfo, config, _dependencyResults, sessionInfo) {
    ...
    try {
      ...
      if (process.env.VISOR_DEBUG === "true") {
        try {
          console.error(`[ai-mock] step=${stepName} mock=${JSON.stringify(mock)}`);
        } catch {
        }
      }
      if (mock !== void 0) {
...
${error.stack || ""}` : String(error);
    ...
    try {
      const inTestMode = process.env.VISOR_TEST_MODE === "true" || process.env.VISOR_TEST_SUBCOMMAND === "true";
      if (!inTestMode && sessionIds.size > 0) {
        const { SessionRegistry: SessionRegistry2 } = (init_session_registry(), __toCommonJS(session_registry_exports));
...
//# sourceMappingURL=chunk-RKV6Y5T4.mjs.map