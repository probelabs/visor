      ...
      async load(specs) {
        ...
        for (const spec of specs) {
          if (spec.name === "ndjson-sink") {
            ...
            const { GitHubFrontend } = await import("./github-frontend-2T5PWYD5.mjs");
            this.frontends.push(new GitHubFrontend());
          } else if (spec.name === "slack") {
            const { SlackFrontend } = await import("./slack-frontend-K3TPGERM.mjs");
            ...
            const { TelegramFrontend } = await import("./telegram-frontend-5UA77YAT.mjs");
            this.frontends.push(new TelegramFrontend(spec.config));
          } else if (spec.name === "email") {
            const { EmailFrontend } = await import("./email-frontend-ECHQCFTR.mjs");
            ...
            const { TeamsFrontend } = await import("./teams-frontend-6RRW533K.mjs");
            this.frontends.push(new TeamsFrontend(spec.config));
          } else if (spec.name === "a2a") {
            const { A2AFrontend } = await import("./a2a-frontend-OI4OVSKC.mjs");
...
//# sourceMappingURL=host-RM6UJEPP.mjs.map