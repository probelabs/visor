  ...
  "src/providers/workflow-check-provider.ts"() {
    ...
    WorkflowCheckProvider = class extends CheckProvider {
      ...
      async loadWorkflowFromConfigPath(sourcePath, baseDir) {
        ...
        const resolved = path6.isAbsolute(sourcePath) ? sourcePath : path6.resolve(baseDir, sourcePath);
        ...
        const id = path6.basename(resolved).replace(/\.(ya?ml)$/i, "");
        const name = loaded.name || `Workflow from ${path6.basename(resolved)}`;
        const workflowDef = {
          id,
          name,
...
async function evaluateIfCondition(checkId, checkConfig, context2, state) {
  ...
  try {
    ...
    const useGlobalOutputs = !!(state.flags && state.flags.forwardRunActive);
    if (useGlobalOutputs) {
      try {
  ...
  if (parentCheckConfig?.on_finish && parentCheckConfig.forEach) {
    ...
    try {
      ...
      if (parentResult) {
        ...
        try {
          const { evaluateTransitions } = await import("./routing-5WFVQA2V.mjs");
        ...
        const { evaluateGoto } = await import("./routing-5WFVQA2V.mjs");
        if (context2.debug) {
          logger.info(
...
var StateMachineExecutionEngine = class _StateMachineExecutionEngine {
  ...
  async executeGroupedChecks(prInfo, checks, timeout, config, outputFormat, debug, maxParallelism, failFast, tagFilter, _pauseGate) {
    ...
    if (!config) {
      const { ConfigManager } = await import("./config-GY3RRGWF.mjs");
    ...
    try {
      if (this.executionContext?.mode?.postGroupedComments && configWithTagFilter?.output?.pr_comment) {
        const { PRReviewer } = await import("./reviewer-7OKO7HGZ.mjs");
  ...
  async evaluateFailureConditions(checkName, reviewSummary, config, previousOutputs, authorAssociation) {
    const { FailureConditionEvaluator: FailureConditionEvaluator2 } = await import("./failure-condition-evaluator-VGZCWNRL.mjs");
...
//# sourceMappingURL=chunk-NO6BTIZA.mjs.map