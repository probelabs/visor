...
async function handlePlanReady(context2, state, transition) {
  ...
  for (const [checkId, checkConfig] of Object.entries(context2.config.checks || {})) {
    checkDependencies[checkId] = checkConfig.depends_on || [];
  }
...
async function executeSingleCheck(checkId, context2, state, emitEvent, transition) {
  ...
  try {
    ...
    const dependencyResults = buildDependencyResults(checkId, checkConfig, context2, state);
    const prInfo = {
      number: 1,
  ...
  "src/state-machine/runner.ts"() {
    ...
    StateMachineRunner = class {
      ...
      transition(newState) {
        ...
        this.emitEvent({ type: "StateTransition", from: oldState, to: newState });
        if (this.context.debug) {
      ...
      /**
       * Bubble an event to parent context (M3: nested workflows)
  ...
  "src/providers/workflow-check-provider.ts"() {
    ...
    WorkflowCheckProvider = class extends CheckProvider {
      ...
      async executeViaStateMachine(workflow, inputs, config, prInfo, dependencyResults, context2) {
        ...
        let totalScore = 0;
        let scoreCount = 0;
        for (const [_stepId, stepResult] of Object.entries(result.results)) {
...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-OPTDRNOW.mjs");
...
${expr}`;
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    ...
    try {
      const { emitMermaidFromMarkdown } = await import("./mermaid-telemetry-E7INPJWM.mjs");
...
//# sourceMappingURL=chunk-QWFDBJS5.mjs.map