...
async function handleLevelDispatch(context2, state, transition, emitEvent) {
  ...
  const nonForEachResults = results.filter((r) => {
    if (r.result.isForEach) {
      console.log(`[LevelDispatch] Filtering out forEach result for ${r.checkId}`);
      return false;
    }
    return true;
  });
  console.log(`[LevelDispatch] Calling updateStats for ${nonForEachResults.length} non-forEach results: ${nonForEachResults.map((r) => r.checkId).join(", ")}`);
...
async function executeCheckWithForEachItems(checkId, forEachParent, forEachItems, context2, state, emitEvent, transition) {
  ...
  try {
    ...
    aggStats.totalRuns++;
    const hasFatal = hasFatalIssues(aggregatedResult);
  ...
  if (parentCheckConfig?.on_finish && parentCheckConfig.forEach) {
    ...
    try {
      ...
      if (parentResult) {
        ...
        if (onFinish.run && onFinish.run.length > 0) {
          for (const targetCheck of onFinish.run) {
            if (checkLoopBudget(context2, state, "on_finish", "run")) {
              ...
              aggregatedResult.issues = [...aggregatedResult.issues || [], errorIssue];
        ...
        const { evaluateGoto } = await import("./routing-76ECLAFD.mjs");
        ...
        if (gotoTarget) {
          if (queuedForward && gotoTarget === forEachParent) {
            logger.info(
              `[LevelDispatch] on_finish.goto to self (${gotoTarget}) deferred due to pending on_finish.run`
            );
          } else {
  ...
  try {
    ...
    try {
      context2.journal.commitEntry({
        sessionId: context2.sessionId,
        checkId,
        result: enrichedResultWithTimestamp,
        event: context2.event || "manual",
        scope
      });
    } catch (error) {
    ...
    if (isForEach) {
      try {
        ...
        console.log(`[forEach parent stats] ${checkId}: incrementing totalRuns from ${aggStats.totalRuns} to ${aggStats.totalRuns + 1}`);
        aggStats.totalRuns++;
...
function updateStats(results, state, isForEachIteration = false) {
  for (const { checkId, result, error, duration } of results) {
    ...
    if (stats.skipped) {
      console.log(`[updateStats] ${checkId} was marked as skipped, but now executing - clearing skipped flag`);
      stats.skipped = false;
    }
    stats.totalRuns++;
    console.log(`[updateStats] ${checkId}: totalRuns now ${stats.totalRuns}, isForEachIteration=${isForEachIteration}`);
    if (duration !== void 0) {
  ...
  "src/state-machine/runner.ts"() {
    ...
    StateMachineRunner = class {
      ...
      buildExecutionResult() {
        ...
        console.log("[StateMachine][Stats] Final statistics breakdown:");
        for (const s of stats) {
          console.log(`  ${s.checkName}: totalRuns=${s.totalRuns}, successful=${s.successfulRuns}, failed=${s.failedRuns}`);
        }
        console.log(`[StateMachine][Stats] Total: ${this.state.stats.size} configured, ${stats.reduce((sum, s) => sum + s.totalRuns, 0)} executions`);
        return {
          results,
          statistics: {
...
var StateMachineExecutionEngine = class _StateMachineExecutionEngine {
  ...
  async executeGroupedChecks(prInfo, checks, timeout, config, outputFormat, debug, maxParallelism, failFast, tagFilter, _pauseGate) {
    ...
    try {
      if (this.executionContext?.mode?.postGroupedComments && configWithTagFilter?.output?.pr_comment) {
        const { PRReviewer } = await import("./reviewer-RNGL55EW.mjs");
  ...
  async evaluateFailureConditions(checkName, reviewSummary, config, previousOutputs, authorAssociation) {
    const { FailureConditionEvaluator: FailureConditionEvaluator2 } = await import("./failure-condition-evaluator-VQK5RPQR.mjs");
...
//# sourceMappingURL=chunk-JRELAV6D.mjs.map