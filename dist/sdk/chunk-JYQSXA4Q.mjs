...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-YM74GNSF.mjs");
...
${expr}
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    ...
    const eventName = (() => {
      try {
        const anyInfo = _prInfo;
        return anyInfo?.eventContext?.event_name || anyInfo?.eventType || "manual";
      } catch {
        return "manual";
      }
    })();
    const eventAction = (() => {
      try {
        const anyInfo = _prInfo;
        return anyInfo?.eventContext?.action;
      } catch {
        return void 0;
      }
    })();
    const templateData = {
      ...
      // Minimal event context for permission and intent-aware rendering
      authorAssociation,
      event: { name: eventName, action: eventAction }
    };
...
//# sourceMappingURL=chunk-JYQSXA4Q.mjs.map