...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-MDA75C3L.mjs");
...
${js}
                  ...
                  for (const stepId of childRunList) {
                    ...
                    const childOut = childRes?.output;
                    this.recordIterationComplete(
                      stepId,
                      childStart,
                      childSuccess,
                    ...
                    try {
                      if (childOut !== void 0) this.trackOutputHistory(stepId, childOut);
                    } catch {
                    }
                  }
                }
              ...
              try {
                const issues = memoryHelpers.get("fact_validation_issues", "fact-validation");
...
${onFinish.goto_js}
        ...
        try {
          const mem = MemoryStore.getInstance(this.config?.memory);
          const issues = mem.get("fact_validation_issues", "fact-validation");
          if (Array.isArray(issues) && issues.length > 0) {
            const stepId = "comment-assistant";
            logger.info(
              `\u25B6 on_finish.post: scheduling '${stepId}' due to validation issues in memory (${issues.length})`
            );
            if (!this.executionStats.has(stepId)) this.initializeCheckStats(stepId);
            const childStart = this.recordIterationStart(stepId);
            const childRes = await this.runNamedCheck(stepId, [], {
            ...
            for (const stepId of Array.from(new Set(runList))) {
              ...
              if (!inItem && mode === "map" && items.length > 0) {
                for (let i = 0; i < items.length; i++) {
                  ...
                  const childIssues = (childRes.issues || []).map((i2) => ({ ...i2 }));
                  const childSuccess = !this.hasFatal(childIssues);
                  const childOut = childRes?.output;
                  this.recordIterationComplete(
                    stepId,
                  ...
                  try {
                    const out = childRes?.output;
                    if (out !== void 0) this.trackOutputHistory(stepId, out);
                  } catch {
                  }
                }
              } else {
...
//# sourceMappingURL=chunk-KTTIHFD7.mjs.map