...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-ZL6UF64K.mjs");
...
var AICheckProvider = class extends CheckProvider {
  ...
  async executeWithConfig(prInfo, config, _dependencyResults, sessionInfo) {
    ...
    try {
      ...
      sessionInfo?.hooks?.onPromptCaptured?.({ step: String(stepName), provider: "ai", prompt: processedPrompt });
    } catch {
    }
    try {
      const stepName = config.checkName || "unknown";
...
//# sourceMappingURL=chunk-EZSB4PQE.mjs.map