...
} from "./chunk-YP6O3YTM.mjs";
import {
  DependencyResolver,
  WorkflowRegistry,
  init_dependency_resolver,
  init_workflow_registry
} from "./chunk-HTAT2O5E.mjs";
import {
  AIReviewService,
...
async function handlePlanReady(context2, state, transition) {
  if (context2.debug) {
    ...
    if (context2.config.tag_filter) {
      logger.info(`[PlanReady] Tag filter: include=${JSON.stringify(context2.config.tag_filter.include)}, exclude=${JSON.stringify(context2.config.tag_filter.exclude)}`);
    } else {
  ...
  for (const [checkId, checkConfig] of Object.entries(context2.config.checks || {})) {
    ...
    if (!requestedChecksSet && routingRunTargets.has(checkId)) {
      if (context2.debug) {
        logger.info(`[PlanReady] Skipping check '${checkId}': routing-run target (will be scheduled by on_*.run)`);
      }
      continue;
    }
  ...
  if (!context2.requestedChecks || context2.requestedChecks.length === 0) {
    ...
    while (queue.length > 0) {
      ...
      for (const child of kids) {
        ...
        queue.push(child);
        if (context2.debug) logger.info(`[PlanReady] Added dependent '${child}' via forward-closure from '${cur}'`);
      }
    }
  }
    ...
    const dependencies = (checkConfig.depends_on || []).flatMap((d) => {
      if (typeof d === "string" && d.includes("|")) {
        ...
        if (tagFilter && finalChecks[d] === void 0) {
          if (context2.debug) {
            logger.info(`[PlanReady] Soft dependency '${d}' of check '${checkId}' filtered out by tags - check will run without it`);
          }
          return [];
        }
        return [d];
...
async function handleWavePlanning(context2, state, transition) {
  ...
  if (forwardRunRequests.length > 0 && (state.levelQueue.length === 0 || forwardRunRequests.some((r) => r.origin === "goto" || r.origin === "goto_js"))) {
    ...
    if (checksToRun.size > 0) {
      ...
      for (const checkId of subgraphChecks) {
        ...
        const depList = Array.isArray(deps) ? deps : [deps];
        subDeps[checkId] = depList.filter((d) => checksToRun.has(d));
      }
  ...
  if (waveRetryEvents.length > 0 && state.levelQueue.length === 0 && !state.eventQueue.some((e) => e.type === "ForwardRunRequested")) {
    ...
    for (const [name, stats] of state.stats.entries()) {
      logger.info(`[WavePlanning] Check ${name}: skipped=${stats.skipped}, skipReason=${stats.skipReason}`);
...
function findTransitiveDependents(target, context2, gotoEvent) {
  ...
  const checks = context2.config.checks || {};
  if (context2.debug) {
    logger.info(`[WavePlanning] findTransitiveDependents called for target=${target}, gotoEvent=${gotoEvent}`);
  }
  ...
  "src/providers/workflow-check-provider.ts"() {
    ...
    WorkflowCheckProvider = class extends CheckProvider {
      ...
      async executeViaStateMachine(workflow, inputs, config, prInfo, dependencyResults, context2) {
        const { projectWorkflowToGraph: projectWorkflowToGraph2, validateWorkflowDepth: validateWorkflowDepth2 } = (init_workflow_projection(), __toCommonJS(workflow_projection_exports));
...
async function handleLevelDispatch(context2, state, transition, emitEvent) {
  ...
  const nonForEachResults = results.filter((r) => {
    if (r.result.isForEach) {
      return false;
    }
    const checkStats = state.stats.get(r.checkId);
...
async function executeCheckGroup(checks, context2, state, maxParallelism, emitEvent, transition) {
  ...
  for (const checkId of uniqueChecks) {
    ...
    const promise = (async () => {
      const startTime = Date.now();
      try {
        const result = await executeSingleCheck(checkId, context2, state, emitEvent, transition);
...
async function executeCheckWithForEachItems(checkId, forEachParent, forEachItems, context2, state, emitEvent, transition) {
  ...
  logger.info(`[LevelDispatch][DEBUG] executeCheckWithForEachItems: checkId=${checkId}, forEachParent=${forEachParent}, items=${forEachItems.length}`);
  ...
  for (let itemIndex = 0; itemIndex < forEachItems.length; itemIndex++) {
    ...
    logger.info(`[LevelDispatch][DEBUG] Starting iteration ${itemIndex} of ${checkId}, parent=${forEachParent}, item=${JSON.stringify(forEachItem)?.substring(0, 100)}`);
    ...
    try {
      ...
      if (!content && enrichedIssues.length > 0) {
        content = enrichedIssues.map((i) => `- **${i.severity.toUpperCase()}**: ${i.message} (${i.file}:${i.line})`).join("\n");
      }
      ...
      logger.info(`[LevelDispatch][DEBUG] Iteration ${itemIndex}: output=${JSON.stringify(output)?.substring(0, 100)}, hasFatalIssues=${iterationHasFatalIssues}`);
      ...
      if (checkConfig.fail_if) {
        try {
          ...
          if (failed) {
            logger.warn(`[LevelDispatch] fail_if triggered for ${checkId} iteration ${itemIndex}: ${checkConfig.fail_if}`);
            const failIssue = {
              file: "system",
              line: 0,
          ...
          const msg = error instanceof Error ? error.message : String(error);
          logger.error(`[LevelDispatch] Error evaluating fail_if for ${checkId} iteration ${itemIndex}: ${msg}`);
        }
      }
      ...
      try {
        ...
        logger.info(`[LevelDispatch][DEBUG] Committing to journal: checkId=${checkId}, scope=${JSON.stringify(scope)}, hasOutput=${output !== void 0}`);
      ...
      const iterationDurationMs = Date.now() - iterationStartMs;
      perIterationDurations.push(iterationDurationMs);
      updateStats([{ checkId, result: enrichedResult, duration: iterationDurationMs }], state, true);
    } catch (error) {
      const iterationDurationMs = Date.now() - iterationStartMs;
      perIterationDurations.push(iterationDurationMs);
      const err = error instanceof Error ? error : new Error(String(error));
      ...
      updateStats([{ checkId, result: { issues: [errorIssue] }, error: err, duration: iterationDurationMs }], state, true);
    }
  }
  ...
  logger.info(`[LevelDispatch][DEBUG] Aggregated result for ${checkId}: forEachItems.length=${allOutputs.length}, results=${perItemResults.length}`);
  ...
  logger.info(`[LevelDispatch][DEBUG] Checking on_finish for forEach parent ${forEachParent}: has_on_finish=${!!parentCheckConfig?.on_finish}, is_forEach=${!!parentCheckConfig?.forEach}`);
      ...
      if (parentResult) {
        ...
        if (onFinish.run && onFinish.run.length > 0) {
          for (const targetCheck of onFinish.run) {
            ...
            logger.info(`[LevelDispatch] Loop budget check: routingLoopCount=${state.routingLoopCount}, max_loops=${context2.config.routing?.max_loops ?? 10}`);
        ...
        const { evaluateGoto } = await import("./routing-2REV6SBB.mjs");
        if (context2.debug || true) {
          ...
          const outputs_history = state.outputs_history || {};
          logger.info(`[LevelDispatch] outputs_history['validate-fact']: ${JSON.stringify(outputs_history["validate-fact"] || "undefined")}`);
          logger.info(`[LevelDispatch] outputs_history keys: ${Object.keys(outputs_history).join(", ")}`);
        }
        ...
        if (queuedForward) {
          ...
          logger.info(`[LevelDispatch] Checking WaveRetry guard: ${guardKey}, has=${!!state.forwardRunGuards?.has(guardKey)}`);
          ...
          logger.info(`[LevelDispatch] queuedForward=false, skipping WaveRetry emission`);
        }
      } else {
        logger.warn(`[LevelDispatch] Could not find parent result for ${forEachParent} in journal`);
      }
    } catch (error) {
      logger.error(`[LevelDispatch] Error processing on_finish for forEach parent ${forEachParent}: ${error}`);
    }
  }
  return aggregatedResult;
}
async function executeSingleCheck(checkId, context2, state, emitEvent, transition) {
  const checkConfig = context2.config.checks?.[checkId];
  if (checkConfig?.if) {
    const shouldRun = await evaluateIfCondition(checkId, checkConfig, context2, state);
    if (!shouldRun) {
      logger.info(`\u23ED  Skipped ${checkId} (if: ${checkConfig.if.substring(0, 40)}${checkConfig.if.length > 40 ? "..." : ""})`);
      const emptyResult = { issues: [] };
      try {
        Object.defineProperty(emptyResult, "__skipped", { value: "if_condition", enumerable: false });
      } catch {
      }
      state.completedChecks.add(checkId);
      const stats = {
  ...
  for (const depId of depList) {
    if (!depId) continue;
    const depStats = state.stats.get(depId);
    const depFailed = failedChecks && failedChecks.has(depId);
    if (depStats?.skipped || depFailed) {
      let skipReason;
      if (depStats?.skipped) {
        skipReason = depStats.skipReason === "forEach_empty" ? "forEach_empty" : "dependency_failed";
      } else if (depFailed) {
        skipReason = "dependency_failed";
      } else {
        skipReason = "dependency_failed";
      }
      logger.info(`\u23ED  Skipped ${checkId} (dependency "${depId}" was ${depStats?.skipped ? "skipped" : "failed"}: ${skipReason})`);
      const emptyResult = { issues: [] };
      try {
        Object.defineProperty(emptyResult, "__skipped", { value: skipReason, enumerable: false });
      } catch {
      }
      state.completedChecks.add(checkId);
      if (!state.failedChecks) {
  ...
  if (forEachParent && forEachItems !== void 0) {
    ...
    if (fanoutMode === "map") {
      if (forEachItems.length === 0) {
        logger.info(`\u23ED  Skipped ${checkId} (forEach parent "${forEachParent}" has 0 items)`);
        if (context2.debug) {
  ...
  try {
    ...
    logger.info(`[LevelDispatch][DEBUG] After execution ${checkId}: checkConfig.forEach=${checkConfig2.forEach}, output type=${typeof result.output}, isArray=${Array.isArray(result.output)}`);
    if (checkConfig2.forEach === true) {
      const output = result.output;
      logger.info(`[LevelDispatch][DEBUG] Processing forEach=true for ${checkId}, output=${JSON.stringify(output)?.substring(0, 200)}`);
      if (output === void 0) {
    ...
    if (!renderedContent && enrichedIssues.length > 0) {
      renderedContent = enrichedIssues.map((i) => `- **${i.severity.toUpperCase()}**: ${i.message} (${i.file}:${i.line})`).join("\n");
    }
...
function buildDependencyResultsWithScope(checkId, checkConfig, context2, scope) {
  ...
  for (const depId of depList) {
    ...
    try {
      ...
      let journalResult = contextView.get(depId);
...
function updateStats(results, state, isForEachIteration = false) {
  for (const { checkId, result, error, duration } of results) {
    ...
    if (checkId === "post-response") {
      logger.info(`[updateStats] Called for post-response: existing.skipped=${existing?.skipped}, stats.skipped=${stats.skipped}, skipReason=${stats.skipReason}`);
    }
    if (stats.skipped) {
      stats.skipped = false;
      if (checkId === "post-response") {
        logger.info(`[updateStats] Clearing skipped flag for post-response (was skipped, now executing)`);
      }
    }
    stats.totalRuns++;
    if (duration !== void 0) {
    ...
    StateMachineRunner = class {
      ...
      buildExecutionResult() {
        ...
        if (this.context.debug) {
          ...
          for (const s of stats) {
            logger.info(`  ${s.checkName}: totalRuns=${s.totalRuns}, successful=${s.successfulRuns}, failed=${s.failedRuns}`);
          }
          logger.info(`[StateMachine][Stats] Total: ${this.state.stats.size} configured, ${stats.reduce((sum, s) => sum + s.totalRuns, 0)} executions`);
        }
        return {
          results,
          statistics: {
          ...
          for (const group of Object.keys(groupedResults)) {
            for (const checkResult of groupedResults[group]) {
              if (checkResult.issues && checkResult.issues.length > 0) {
                checkResult.issues = filter.filterIssues(checkResult.issues, this.context.workingDirectory);
              }
            }
          }
        }
...
var StateMachineExecutionEngine = class _StateMachineExecutionEngine {
  ...
  async executeChecks(options) {
    ...
    try {
      if (options.config?.memory) {
        const { MemoryStore: MemoryStore2 } = await import("./memory-store-JQ22JCTL.mjs");
  ...
  async executeGroupedChecks(prInfo, checks, timeout, config, outputFormat, debug, maxParallelism, failFast, tagFilter, _pauseGate) {
    ...
    if (!config) {
      const { ConfigManager } = await import("./config-NWIVBVY6.mjs");
    ...
    try {
      if (this.executionContext?.mode?.postGroupedComments && configWithTagFilter?.output?.pr_comment) {
        const { PRReviewer } = await import("./reviewer-TOT2YRSV.mjs");
        const reviewer = new PRReviewer(this._lastContext?.executionContext?.octokit);
        let owner;
        let repo;
        try {
  ...
  buildEngineContext(config, prInfo, debug, maxParallelism, failFast, requestedChecks) {
    ...
    for (const [checkId, checkConfig] of Object.entries(config.checks || {})) {
      checks[checkId] = {
        ...
        triggers: (Array.isArray(checkConfig.on) ? checkConfig.on : [checkConfig.on]).filter(Boolean),
        group: checkConfig.group,
  ...
  getOutputHistorySnapshot() {
    ...
    if (!journal) {
      logger.info("[StateMachine][DEBUG] getOutputHistorySnapshot: No journal found");
      return {};
    }
    const sessionId = this._lastContext?.sessionId;
    if (!sessionId) {
      logger.info("[StateMachine][DEBUG] getOutputHistorySnapshot: No sessionId found");
      return {};
    }
    const snapshot = journal.beginSnapshot();
    const allEntries = journal.readVisible(sessionId, snapshot, void 0);
    logger.info(`[StateMachine][DEBUG] getOutputHistorySnapshot: Found ${allEntries.length} journal entries`);
    const outputHistory = {};
    for (const entry of allEntries) {
      const checkId = entry.checkId;
    ...
    logger.info(`[StateMachine][DEBUG] getOutputHistorySnapshot result: ${JSON.stringify(Object.keys(outputHistory))}`);
    for (const [checkId, outputs] of Object.entries(outputHistory)) {
      logger.info(`[StateMachine][DEBUG]   ${checkId}: ${outputs.length} outputs`);
    }
    return outputHistory;
  }
  /**
  ...
  async evaluateFailureConditions(checkName, reviewSummary, config, previousOutputs, authorAssociation) {
    const { FailureConditionEvaluator: FailureConditionEvaluator2 } = await import("./failure-condition-evaluator-KQD7KDJC.mjs");
  ...
  async renderCheckContent(checkName, reviewSummary, checkConfig, _prInfo) {
    const { createExtendedLiquid: createExtendedLiquid2 } = await import("./liquid-extensions-MSDKNYCC.mjs");
...
//# sourceMappingURL=chunk-G6RHIP6V.mjs.map