...
async function handlePlanReady(context2, state, transition) {
  ...
  for (const [checkId, checkConfig] of Object.entries(context2.config.checks || {})) {
    const dependencies = (checkConfig.depends_on || []).flatMap(
...
async function executeSingleCheck(checkId, context2, state, emitEvent, transition) {
  ...
  try {
    ...
    const isForEach = result.isForEach;
    const forEachItems2 = result.forEachItems;
    if (isForEach) {
      enrichedResult.isForEach = true;
    }
    if (forEachItems2) {
      enrichedResult.forEachItems = forEachItems2;
    }
...
var PRReviewer = class {
  ...
  async reviewPR(owner, repo, prNumber, prInfo, options = {}) {
    ...
    if (config && checks && checks.length > 0) {
      const { CheckExecutionEngine: CheckExecutionEngine2 } = await import("./check-execution-engine-S7GFYLBK.mjs");
...
//# sourceMappingURL=chunk-ED2ITIP6.mjs.map