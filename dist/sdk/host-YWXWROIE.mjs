      ...
      async load(specs) {
        ...
        for (const spec of specs) {
          if (spec.name === "ndjson-sink") {
            ...
            const { SlackFrontend } = await import("./slack-frontend-H7PX5GE7.mjs");
            ...
            const { TelegramFrontend } = await import("./telegram-frontend-YQRYDKPN.mjs");
            this.frontends.push(new TelegramFrontend(spec.config));
          } else if (spec.name === "email") {
            const { EmailFrontend } = await import("./email-frontend-JE7H3LBC.mjs");
            ...
            const { TeamsFrontend } = await import("./teams-frontend-HCT4MTY4.mjs");
            this.frontends.push(new TeamsFrontend(spec.config));
          } else if (spec.name === "a2a") {
            const { A2AFrontend } = await import("./a2a-frontend-7EGXSULE.mjs");
...
//# sourceMappingURL=host-YWXWROIE.mjs.map