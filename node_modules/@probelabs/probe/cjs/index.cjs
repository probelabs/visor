...
function setBinaryPath(binaryPath) {
  if (!import_fs_extra3.default.existsSync(binaryPath)) {
...
function createSearchTool() {
  return {
    ...
    description: searchDescription,
    schema: searchSchema,
    func: async ({ query: searchQuery, path: path6, allow_tests, exact, maxResults, maxTokens = 1e4, language }) => {
      try {
        const results = await search({
          query: searchQuery,
          path: path6,
...
function createQueryTool() {
  return {
    ...
    description: queryDescription,
    schema: querySchema,
    func: async ({ pattern, path: path6, language, allow_tests }) => {
      try {
        const results = await query({
          pattern,
          path: path6,
