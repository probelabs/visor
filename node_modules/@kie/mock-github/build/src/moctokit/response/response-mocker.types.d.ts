...
type ExtractStatus<Responses> = keyof Responses extends number ? keyof Responses : never;
type ExtractResponseData<Responses> = {
    [K in ExtractStatus<Responses>]: GetContentKeyIfPresent<Responses[K]> extends never ? never : MockResponse<GetContentKeyIfPresent<Responses[K]>, K>;
}[ExtractStatus<Responses>];
export type ExtractMockResponse<R> = "responses" extends keyof R ? ExtractResponseData<R["responses"]> : never;
export {};
