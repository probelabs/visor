...
export type Parameters<T> = "parameters" extends keyof T ? UnionToIntersection<{
    [K in keyof T["parameters"]]: T["parameters"][K];
}[keyof T["parameters"]]> : Record<never, never>;
type RequestBody<T> = "requestBody" extends keyof T ? "content" extends keyof NonNullable<T["requestBody"]> ? "application/json" extends keyof NonNullable<T["requestBody"]>["content"] ? NonNullable<T["requestBody"]>["content"]["application/json"] : {
    data: {
        [K in keyof NonNullable<T["requestBody"]>["content"]]: NonNullable<T["requestBody"]>["content"][K];
    }[keyof NonNullable<T["requestBody"]>["content"]];
} : "application/json" extends keyof NonNullable<T["requestBody"]> ? NonNullable<T["requestBody"]>["application/json"] : {
    data: {
