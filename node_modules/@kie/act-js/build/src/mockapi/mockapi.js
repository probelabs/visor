...
class Mockapi {
        ...
        for (const apiName in this.apiSchema) {
            ...
            for (const scope in this.apiSchema[apiName]["endpoints"]) {
                ...
                for (const methodName in this.apiSchema[apiName]["endpoints"][scope]) {
                    methods[apiName][scope][methodName] = new request_mocker_1.MockapiRequestMocker(this.apiSchema[apiName]["baseUrl"], this.apiSchema[apiName]["endpoints"][scope][methodName], this.allowUnmocked).request;
                }
            }
        }
        return methods;
    ...
    validateAPISchema(apiSchema) {
        const rawJSON = typeof apiSchema === "string"
