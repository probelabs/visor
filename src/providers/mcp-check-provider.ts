...
import {
  extractIssuesFromOutput as sharedExtractIssues,
  normalizeIssue as sharedNormalizeIssue,
  normalizeIssueArray as sharedNormalizeIssueArray,
} from '../utils/issue-normalizer';
...
export class McpCheckProvider extends CheckProvider {
  ...
  private extractIssuesFromOutput(
    ...
    return sharedExtractIssues(output);
  }

  private normalizeIssueArray(values: unknown[]): ReviewIssue[] | null {
    return sharedNormalizeIssueArray(values, 'mcp');
  }

  private normalizeIssue(raw: unknown): ReviewIssue | null {
    return sharedNormalizeIssue(raw, 'mcp');
  }
