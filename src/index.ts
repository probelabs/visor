  ...
  switch (command.type) {
    ...
    default:
      ...
      if (commandRegistry[command.type]) {
        ...
        // Optional rerun: if comment-assistant requested rerun and commenter is MEMBER+
        try {
          const intent = (() => {
            for (const group of Object.values(groupedResults)) {
              for (const r of group) {
                if (r.checkName === 'comment-assistant') {
          ...
          const assoc = (context.event as any)?.comment?.author_association as string | undefined;
          const { createPermissionHelpers, detectLocalMode } = await import(
            './utils/author-permissions'
          );
          const perms = createPermissionHelpers(assoc, detectLocalMode());
          const allowed = perms.hasMinPermission('MEMBER');
          const isPRThread = Boolean((context.event as any)?.issue?.pull_request);

          if (isPRThread && allowed && intent === 'comment_retrigger') {
            console.log(
              '▶ Rerun requested by commenter; running full review suite with GitHub Checks...'
            );

            // Fetch PR context with pr_updated to satisfy event filters
            const prInfoRerun = await analyzer.fetchPRDiff(
              owner,
              repo,
              prNumber,
              undefined,
            ...
            (prInfoRerun as any).eventContext = context.event;
            (prInfoRerun as any).includeCodeContext = true;
            (prInfoRerun as any).isPRContext = true;

            const suite = ['overview', 'security', 'performance', 'quality', 'style'].filter(c =>
              Boolean((config as any)?.checks?.[c])
            );

            // Create GitHub check runs for the suite
            const headSha = (context.event as any)?.pull_request?.head?.sha || 'unknown';
            ...
            const suiteResults = await reviewer.reviewPR(owner, repo, prNumber, prInfoRerun, {
              format,
              config: config as import('./types/config').VisorConfig,
              checks: suite,
            ...
            if (shouldComment) {
              const commenterLogin = (context.event as any)?.comment?.user?.login || 'unknown';
              await reviewer.postReviewComment(owner, repo, prNumber, suiteResults, {
                format,
                triggeredBy: `issue_comment by @${commenterLogin}`,
              });
            }
          }
        } catch (e) {
          console.warn('⚠️ Rerun scheduling failed:', e instanceof Error ? e.message : String(e));
        }
      }
      break;
  }
