...
npx -y visor@latest --config examples/routing-basic.yaml --output table

# on_success: run notify and jump back once to re-run unit-tests
npx -y visor@latest --config examples/routing-on-success.yaml --output table

# forEach remediation: mark missing items then retry
npx -y visor@latest --config examples/routing-foreach.yaml --output table

# Dynamic routing with *_js hooks
npx -y visor@latest --config examples/routing-dynamic-js.yaml --output table
```
