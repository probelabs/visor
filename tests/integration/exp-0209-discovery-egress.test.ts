  ...
  it('settles both new managed Proof providers and reaps their process groups', async () => {
    await withProofFixture(async (proof, root) => {
      ...
      const admissionReceipt = {
        ...
        CandidateID: 'sha256:' + '1'.repeat(64), ProbeResultDigest: 'sha256:' + '2'.repeat(64), ProbeCanonicalBytes: Buffer.byteLength(canonicalJson(candidatePayload)),
        ClaimID: candidate.claimId, Claim: candidate.claim, PayloadFingerprint: candidate.payloadFingerprint,
        InvocationDigest: 'sha256:' + '4'.repeat(64), RoleID: 'onboard', Stance: 'owner', Subject: { kind: 'project', id: 'journalservice', fingerprint: 'sha256:' + '5'.repeat(64) },
        ProducerCheckID: 'inspect', ParentClaimIDs: candidate.parentClaimIds,
        Binding: { ManagedRunID: 'managed', SessionID: 'session', CheckID: 'inspect', Scope: [{ Kind: 'keyed', ExpansionOwnerCheck: 'project', Key: 'journalservice', SubgraphInstanceID: 'a'.repeat(64) }], NodeInstanceID: 'node', NodeGenerationID: 'generation', AttemptID: 'attempt', Fence: 1 },
        Termination: { Version: 1, Type: 'ManagedRunTerminated', SessionID: 'session', Scope: [{ Kind: 'keyed', ExpansionOwnerCheck: 'project', Key: 'journalservice', SubgraphInstanceID: 'a'.repeat(64) }], Binding: { ManagedRunID: 'managed', SessionID: 'session', CheckID: 'inspect', Scope: [{ Kind: 'keyed', ExpansionOwnerCheck: 'project', Key: 'journalservice', SubgraphInstanceID: 'a'.repeat(64) }], NodeInstanceID: 'node', NodeGenerationID: 'generation', AttemptID: 'attempt', Fence: 1 }, CleanupStatus: 'clean', ControllerDecision: 'completed', FailureCode: null },
  ...
  it('executes authored providers through the native scheduler and replays independent keyed fanout', async () => {
    await withProofFixture(async (proof, root) => {
      ...
      try {
        ...
        expect(JSON.parse(admissionEvent.payload.__proof_admission_wire)).toEqual(expect.objectContaining({ version: 'proof.role-result-candidate-cli-decision/v1', status: 'ADMITTED', receipt: expect.objectContaining({ Version: 'proof.role-result-candidate-admission/v1', Status: 'ADMITTED' }), reject_code: null }));
