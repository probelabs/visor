...
**CLI:** `npx -y visor@latest --output table`
...
```bash
npx -y visor@latest --output table --debug
```

**2) Try JSON output to a file:**
```bash
npx -y visor@latest --check security --output json --output-file visor-results.json
```

**3) Filter by tags (fast/local):**
```bash
npx -y visor@latest --tags local,fast --max-parallelism 5
```
...
npx -y visor@latest --tags local,fast

# Comprehensive checks
npx -y visor@latest --tags comprehensive
```
...
```bash
npx -y visor@latest --check overview,security --output table
```
...
**Run specific checks:** `npx -y visor@latest --check security,performance`
...
npx -y visor@latest --tags ci,comprehensive
```
...
```bash
npx -y visor@latest --config workshop/labs/lab-01-basic.yaml \
...
npx -y visor@latest --check release-notes --output markdown \
...
```bash
# HTTP integration
npx -y visor@latest --config examples/http-integration-config.yaml \
  --check github-webhook --output table

# Cron webhook
npx -y visor@latest --config examples/cron-webhook-config.yaml \
  --output table

# Jira integration
JIRA_ISSUE=PROJ-123 JIRA_TOKEN=xxx \
npx -y visor@latest --config examples/jira-simple-example.yaml \
  --output markdown
```
...
TAG_NAME=v1.0.0 GIT_LOG="$(git log --oneline -n 20)" \
GIT_DIFF_STAT="$(git diff --stat HEAD~20..HEAD)" \
npx -y visor@latest --config defaults/.visor.yaml \
...
```bash
npx -y visor@latest --config examples/forEach-example.yaml \
  --output table --debug
```
...
```bash
npx -y visor@latest --check security --debug
```
...
npx -y visor@latest --check security \
...
```bash
npx -y visor@latest --config workshop/labs/lab-03-debug.yaml \
...
`env -u GOOGLE_API_KEY -u ANTHROPIC_API_KEY -u OPENAI_API_KEY npx -y visor@latest ...`
...
npx -y visor@latest --output table

# Filter by tags
npx -y visor@latest --tags local,fast

# JSON/SARIF outputs
npx -y visor@latest --check security --output json --output-file results.json
npx -y visor@latest --check security --output sarif --output-file results.sarif

# Use a specific config
npx -y visor@latest --config workshop/labs/lab-01-basic.yaml --tags local,fast

# Debugging
npx -y visor@latest --debug
```
